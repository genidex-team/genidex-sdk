(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.GeniDexSDK = {}, global.ethers));
})(this, (function (exports, ethers) { 'use strict';

    const constants = {
        ETH_ADDRESS: ethers.ethers.ZeroAddress,
        WAD: ethers.ethers.parseEther("1")
    };
    ...
    class Utils {
        ...
        formatNormAmount(normAmount) {
            const amount = typeof normAmount === 'bigint' ? normAmount : ethers.toBigInt(normAmount);
            const formatted = ethers.formatUnits(amount, 18); // returns a string
    ...
    class ERC20 {
        ...
        getContract(signer) {
            ...
            return new ethers.Contract(this.address, ERC20_ABI, runner);
        }
    ...
    class BuyOrders {
         ...
         */
        async placeBuyOrder({ signer, marketId, normPrice, normQuantity, referrer = ethers.ZeroAddress, overrides = {} }) {
        ...
        getBuyOrderIds(sortedBuyOrders, normQuantity) {
            const selectedIds = [];
            let total = 0n;
            const target = ethers.getBigInt(normQuantity);
    ...
    class SellOrders {
         ...
         * @returns TransactionResponse.
         */
        async placeSellOrder({ signer, marketId, normPrice, normQuantity, referrer = ethers.ZeroAddress, overrides = {} }) {
    ...
    class GeniDex {
        ...
        async connect(networkName, provider) {
            ...
            this.contract = new ethers.Contract(this.address, abi, provider);
        ...
        getContract(signer) {
            const runner = signer ?? this.provider;
            return new ethers.Contract(this.address, this.abi, runner);
        }
        /**
        ...
        getFilledOrderIDs(orders) {
            return orders
                .filter(order => ethers.getBigInt(order.quantity) === 0n)
                .map(order => order.id);
        }
        /**
        ...
        */
        ...
        decodeRevertError(err) {
            ...
            try {
                const interfaces = [
                    new ethers.Interface(this.abi),
                    new ethers.Interface(IERC20Errors),
                ];
        ...
        async revertError(err, functionName, args, overrides) {
            ...
            if (fn) {
                let invocation = new ethers.ErrorDescription(fn, fn?.selector, args);
